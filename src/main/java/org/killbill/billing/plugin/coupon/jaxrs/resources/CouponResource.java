...
package org.killbill.billing.plugin.coupon.jaxrs.resources;
...
import org.killbill.billing.plugin.coupon.CouponCallContext;
import org.killbill.billing.plugin.coupon.CouponTenantContext;
...
import org.killbill.billing.plugin.coupon.json.CouponJson;
...
@Singleton
@Path(CouponResource.COUPON_PATH)
public class CouponResource {

    public static final String COUPON_PATH = "/1.0/kb/plugins/coupon";
    ...
    private static final String QUERY_COUPON_CODE = "couponCode";

    private final DateTimeFormatter DATE_TIME_FORMATTER = ISODateTimeFormat.dateTimeParser();

    private final CouponUserApi couponApi;
    protected final Context context;

    @Inject
    public CouponResource(final CouponUserApi meterApi,
                          final Context context) {
        this.couponApi = meterApi;
        this.context = context;
    }

    @GET
    @Produces(APPLICATION_JSON)
    @ApiOperation(value = "Retrieve a coupon by code")
    ...
    public Response getCouponByCode(@QueryParam(QUERY_COUPON_CODE) final String couponCode,
        ...
        Coupon coupon = couponApi.getCouponByCode(couponCode, tenantContext);
        final CouponJson couponJson = new CouponJson(coupon);
        return Response.status(Response.Status.OK).entity(couponJson).build();
    }
...
//                        couponApi.getUsage(output, source, categories, fromTimestamp, toTimestamp, tenantContext);
//                    } else {
//                        final TimeAggregationMode timeAggregationMode = TimeAggregationMode.valueOf(timeAggregationModeString);
//                        couponApi.getUsage(output, timeAggregationMode, source, categories, fromTimestamp, toTimestamp, tenantContext);
//                    }
//                } else {
//                    final Map<String, Collection<String>> metricsPerCategory = retrieveMetricsPerCategory(categoriesAndMetrics);
//                    if (Strings.isNullOrEmpty(timeAggregationModeString)) {
//                        couponApi.getUsage(output, source, metricsPerCategory, fromTimestamp, toTimestamp, tenantContext);
//                    } else {
//                        final TimeAggregationMode timeAggregationMode = TimeAggregationMode.valueOf(timeAggregationModeString);
//                        couponApi.getUsage(output, timeAggregationMode, source, metricsPerCategory, fromTimestamp, toTimestamp, tenantContext);
//                    }
//                }
//            }
//        };
...
//
//        if (withAggregate) {
//            couponApi.incrementUsageAndAggregate(source, categoryName, metricName, timestamp, callContext);
//        } else {
//            couponApi.incrementUsage(source, categoryName, metricName, timestamp, callContext);
//        }
//
...
//            return new CouponCallContext(tenant == null ? null : tenant.getId(), createdBy, CallOrigin.EXTERNAL, UserType.CUSTOMER, reason,
...
//    }
    ...
    private TenantContext createContext(final ServletRequest request) {
        ...
        if (tenant == null) {
            ...
            return new CouponTenantContext();
        } else {
            return new CouponTenantContext(tenant.getId());
        }
    }
